    ...
    public function configure(array $params)
    {
        ...
        switch ($this->codigo_banco) {
            ...
            case \Cnab\Banco::BRADESCO:
                ...
                $this->header->data_geracao = $this->configuracao['data_geracao']->format('dmy');
                $this->header->numero_remessa = $this->configuracao['sequencial_remessa'];
                break;
            case \Cnab\Banco::SICREDI:
                $this->header->codigo_cedente = $this->configuracao['codigo_cedente'];
                $this->header->numero_remessa = $this->configuracao['numero_remessa'];
                $this->header->numero_sequencial = $this->configuracao['numero_sequencial'];
                $this->header->cnpj = $this->prepareText($this->configuracao['cnpj'], '.-/');
            ...
            case \Cnab\Banco::SICOOB:
                ...
                $this->header->codigo_cedente = $this->configuracao['codigo_cedente'];
                $this->header->codigo_cedente_dv = $this->configuracao['codigo_cedente_dv'];
                $this->header->razao_social = $this->prepareText($this->configuracao['razao_social']);
                $this->header->numero_remessa = $this->configuracao['numero_remessa'];
                $this->header->numero_sequencial = $this->configuracao['numero_sequencial'];
                $this->header->data_geracao = $this->configuracao['data_geracao']->format('dmy');
            ...
            default:
                ...
                $this->header->nome_empresa = $this->prepareText($this->configuracao['nome_fantasia']);
                $this->header->data_geracao = $this->configuracao['data_geracao']->format('dmy');
                break;
        }
    }
    ...
    public function insertDetalhe(array $boleto, $tipo = 'remessa')
    {
        ...
        if ($tipo == 'remessa') {
            ...
            if (\Cnab\Banco::BRADESCO == $this->codigo_banco) {
                ...
                /**
                 * Se = 2 considerar percentual de multa.
                 * Se = 0, sem multa.
                 */
                if ($boleto['percentual_multa'] > 0) {
                    $detalhe->campo_multa = 2;
                    $detalhe->percentual_multa = $boleto['percentual_multa'];
                }
            ...
            } elseif (\Cnab\Banco::SICREDI == $this->codigo_banco) {
                $detalhe->codigo_carteira = '';
                $detalhe->seu_numero = $boleto['seu_numero'];
                $detalhe->data_instrucao = $dateCadastro->format('Ymd');
                if ($boleto['juros_de_um_dia'] > 0) {
                    $detalhe->tipo_juros = 'A';
                    $detalhe->juros_um_dia = $boleto['juros_de_um_dia'];
                }
                $detalhe->percentual_multa = $boleto['percentual_multa'];
            } elseif (\Cnab\Banco::SICOOB == $this->codigo_banco) {
                $detalhe->nosso_numero_dv = $boleto['nosso_numero_dv'];
                $detalhe->agencia = $this->header->agencia;
                $detalhe->agencia_dv = $this->header->agencia_dv;
                $detalhe->agencia_cobradora = $this->header->agencia;
                $detalhe->agencia_dv_cobradora = $this->header->agencia_dv;
                $detalhe->codigo_cedente = 000000;
                $detalhe->cedente_numero_inscricao = $this->prepareText($this->configuracao['cnpj'], '.-/');
                $detalhe->conta = $this->configuracao['conta'];
                $detalhe->conta_dv = $this->configuracao['conta_dac'];

                $detalhe->comando_movimento = '01';
                $detalhe->bairro = $this->prepareText($boleto['sacado_bairro']);
                $detalhe->cidade = $this->prepareText($boleto['sacado_cidade']);
                $detalhe->uf = $boleto['sacado_uf'];
                /**
                 * Taxa de mora mês / Taxa de multa
                 * Ex: 022000 = 2,20%
                 */
                $detalhe->multa = $boleto['percentual_multa'];
                $detalhe->juros_um_dia = $boleto['percentual_juros'];
            } else if (\Cnab\Banco::SANTANDER == $this->codigo_banco) {

                $detalhe->cedente_numero_inscricao = $this->prepareText($this->configuracao['cnpj'], '.-/');
                $detalhe->agencia = $this->configuracao['agencia'];
                $detalhe->carteira = $this->configuracao['carteira'];
                if ($detalhe->carteira == 5) {
                    $detalhe->agencia_cobradora = $this->configuracao['agencia']; //so informar se carteira = 5
                }
                $detalhe->conta = $this->configuracao['conta'];
                ...
                if ($boleto['percentual_multa'] > 0) {
                    $detalhe->informar_multa = 4;
                    $detalhe->multa = $boleto['percentual_multa'];
                }
                $detalhe->juros_um_dia = $boleto['juros_de_um_dia'];
            } elseif (\Cnab\Banco::ITAU == $this->codigo_banco) {
                $detalhe->codigo_inscricao = 2;
                $detalhe->numero_inscricao = $this->prepareText($this->configuracao['cnpj'], '.-/');
                $detalhe->agencia = $this->header->agencia;
                ...
                $detalhe->data_mora = $dateJurosMora->format('dmy');
                if ($boleto['percentual_multa'] > 0) {
                    $detalheMulta = new DetalheMulta($this);
                    $detalheMulta->codigo_multa = 2; // 2= Percentual
                    $detalheMulta->data_multa = $dateJurosMora->format('dmY');
                    $detalheMulta->valor_multa = $boleto['percentual_multa'];
            ...
            if (\Cnab\Banco::SICREDI != $this->codigo_banco
                && \Cnab\Banco::SICOOB != $this->codigo_banco
                && \Cnab\Banco::BRADESCO != $this->codigo_banco
                && \Cnab\Banco::SANTANDER != $this->codigo_banco) {

                $detalhe->uso_empresa = isset($boleto['uso_empresa'])
                ? $boleto['uso_empresa']
                : $boleto['nosso_numero'];
                $detalhe->numero_carteira = $boleto['carteira'];
                $detalhe->instrucao1 = $boleto['instrucao1'] ? $boleto['instrucao1'] : "";
                $detalhe->instrucao2 = $boleto['instrucao2'] ? $boleto['instrucao2'] : "";
                $detalhe->bairro = $this->prepareText($boleto['sacado_bairro']);
                $detalhe->cidade = $this->prepareText($boleto['sacado_cidade']);
            ...
            if (\Cnab\Banco::SICOOB != $this->codigo_banco) {
            ...
            $sacado_tipo = $boleto['sacado_tipo'] ? $boleto['sacado_tipo'] : 'cpf';

            if ($sacado_tipo == 'cnpj') {
                $detalhe->sacado_codigo_inscricao = '2';
                if (\Cnab\Banco::BRADESCO == $this->codigo_banco or \Cnab\Banco::SANTANDER == $this->codigo_banco or \Cnab\Banco::SICOOB == $this->codigo_banco) {
                    $detalhe->sacado_codigo_inscricao = '02';
                }
            ...
            if (isset($boleto['juros_de_um_dia'])) {
                $detalhe->juros_um_dia = $boleto['juros_de_um_dia'];
            }
...
//            print_r($detalhe);
    ...
    public function getText()
    {
        ...
        if ($this->codigo_banco === \Cnab\Banco::SICREDI) {
            ...
            $this->trailer->quantidade_parcelas = $this->configuracao['quantidade_parcelas']+2;
        }
