...
class Arquivo implements \Cnab\Remessa\IArquivo
{
    ...
    public function configure(array $params)
    {
        ...
        if ($this->codigo_banco == \Cnab\Banco::BANCO_DO_BRASIL) {
            ...
            $this->headerArquivo->conta_dv = strtoupper($this->configuracao['conta_dv']);
        }
    ...
    public function insertDetalhe(array $boleto, $tipo = 'remessa')
    {
        ...
        if ($this->codigo_banco == \Cnab\Banco::SICOOB) {
            ...
            $detalhe->segmento_p->conta_dv = $this->headerArquivo->conta_dv;
            $detalhe->segmento_p->modalidade = $this->configuracao['modalidade'];
            $detalhe->segmento_p->codigo_carteira = $this->configuracao['codigo_carteira'];
            $detalhe->segmento_p->numero_titulo = $boleto['nosso_numero_completo'];
        }
        /*
        if ($this->codigo_banco != \Cnab\Banco::SICOOB) {
        ...
        $detalhe->segmento_p->forma_cadastramento = $boleto['registrado'] ? 1 : 0; // 1 = Com, 0 = Sem Registro
         ...
         * '3'  =  Isento
         */
        //no sicoob se o valor do juros for 0, não pode enviar o codigo de juros_mora nem a data.
        if ($this->codigo_banco == \Cnab\Banco::SICOOB && $boleto['juros_de_um_dia']==0){
        ...
        //adiciona verificação do tipo o sacado, quando cnpj codigo de inscricao deve ser 2, quando cpf deve ser 1
        if ($boleto['sacado_tipo'] == 'cnpj' && !empty($boleto['sacado_cnpj'])) {
        ...
        if ($boleto['percentual_multa'] > 0) {
            $detalhe->segmento_r->codigo_multa = 2; //2-percentual
            $detalhe->segmento_r->valor_multa = $boleto['percentual_multa'];
            $detalhe->segmento_r->data_multa = $dateJurosMora;
        } else {
    ...
    public function getText()
    {
        ...
        if ($this->codigo_banco == \Cnab\Banco::CEF || $this->codigo_banco == \Cnab\Banco::SICOOB) {
